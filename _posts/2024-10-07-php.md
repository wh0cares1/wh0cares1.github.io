...
- [**File Signature](https://en.wikipedia.org/wiki/List_of_file_signatures) ([Magic Bytes](https://www.netspi.com/blog/technical-blog/web-application-pentesting/magic-bytes-identifying-common-file-formats-at-a-glance/))**: File types often have unique identifiers, like GIF images (`GIF87a` or `GIF89a`). You might manipulate these signatures to bypass validation, e.g., by adding harmless headers to malicious files.
...
- Error messages may leak information about upload directories, making it easier for attackers to find and execute uploaded files.

### OOB XXE (Out-of-Band XML External Entity Injection)
- OOB XXE exploits can be used for out-of-band (OOB) interaction by causing the server to make HTTP requests to a controlled server, bypassing standard defenses.
...
- Readmore : [Creating Phar Files](https://blog.programster.org/creating-phar-files)
- Rename the file extension to bypass restrictions (`mv payload.phar payload.jpg`).
- Trigger the vulnerability: `phar://path/to/payload.jpg/resource`
