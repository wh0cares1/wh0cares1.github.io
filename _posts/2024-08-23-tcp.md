...
- Data that can be transferred from Application Layer equals MTU minus (Size TCP Header + Size IP Layer), which is 1500 bytes - (20 bytes + 20 bytes) = 1460 bytes.
...
- Calculate using formula -ve of (sum of header + data). Read more: [TCP Checksum](https://www.geeksforgeeks.org/calculation-of-tcp-checksum/)
...
- Receiver use sack to determine which packet that is lost, then sender resend the packet with the same sequence number as the sack.
